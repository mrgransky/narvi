...
for sq in sequences:
	...
	GT_pose_path = '{}{}.npy'.format(par.pose_dir, sq)
	gt = np.load(GT_pose_path)
	pose_result_path = '{}out_{}.txt'.format(predicted_result_dir, sq)
	...
	if gradient_color:
		...
		for st in range(0, len(out), step):
			...
			if st == 0:
				plt.legend()
			plt.title('sq {}'.format(sq))
			save_name = '{}route_{}_gradient.png'.format(predicted_result_dir, sq)
		...
		plt.legend()
		plt.title('sq {}'.format(sq))
		save_name = '{}route_{}.png'.format(predicted_result_dir, sq)
		plt.savefig(save_name)
